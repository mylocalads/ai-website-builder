...
const services = defineCollection({
  ...
  schema: z.object({
    ...
    // Overrides the generated closing-CTA headline ("Need {title} in {city}?").
    // For titles that read badly in that pattern — "Need Roof Inspection in
    // Dayton?" wants an article — set this instead of contorting the title,
    // which is also the nav label and the page H1.
