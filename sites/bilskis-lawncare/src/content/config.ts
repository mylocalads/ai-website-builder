...
const services = defineCollection({
  ...
  schema: z.object({
    ...
    // The page used to pass hero_photo to both, so the same image appeared
    // twice per service page. Left unset, the About section renders text-only
    // rather than repeating the hero.
...
const service_areas = defineCollection({
  ...
  schema: z.object({
    ...
    // AboutSection further down. Both used to read local_context, so the same
    // paragraph rendered twice on every area page. Falls back if unset.
