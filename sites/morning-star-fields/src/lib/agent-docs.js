...
export function llmsTxt({ site, services = [], areas = [] }) {
  ...
  /* The label for the /book route. Was the literal 'Free estimate' in three
     places, which is correct for a contractor and wrong for anyone else — an
     egg producer does not quote estimates, and llms.txt is read verbatim by an
     LLM answering a customer. Falls back to the old wording so contractor
     sites are unchanged. */
  const bookLabel = site.closing_cta?.cta_text ?? 'Free estimate';
...
- [Home](${u('/')})
- [About](${u('/about')})
- [BioCircular farming](${u('/biocircular')})
- [Our work](${u('/our-work')})
- [Pricing](${u('/pricing')})
- [${bookLabel}](${u('/book')})
- [Blog](${u('/blog')})
...
${bookLabel}: ${u('/book')} · ${site.phone_display ?? site.phone ?? ''}
`;
}
...
export function agentSiteSummary({ site, services = [], areas = [] }) {
  const u = (p) => abs(site.site_url, p);
  const where = marketLocation(site);
  /* Same reason as llmsTxt/indexMd: 'Free Estimates' is contractor wording and
     this summary is read verbatim by agents. */
  const bookLabel = site.closing_cta?.cta_text ?? 'Free Estimates';
...
${line('Business Name', site.business_name)}${line('Phone', site.phone_display ?? site.phone)}${line(
    ...
    [site.address?.street, site.address?.city, site.address?.state, site.address?.postal]
      .filter(Boolean)
      .join(', ')
  )}${line('Service Area', where)}${line(bookLabel, u('/book'))}
- **Services**:
