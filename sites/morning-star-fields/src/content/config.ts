...
const services = defineCollection({
  ...
  schema: z.object({
    ...
    // Optional video section on a service page, rendered under the About block.
    // `src` is hotlinked deliberately for large files — see the note in
    // src/content/site/home.json's video_section.
    video: z.object({
      src: z.string(),
...
const blog = defineCollection({
  ...
  schema: z.object({
    ...
    // Describes hero_image. Without it the blog hero and every /blog card
    // rendered alt="", which reads as "decorative" to a screen reader for a
    // photograph that is carrying the article.
  ...
  schema: z.discriminatedUnion('kind', [
    z.object({
      ...
      partners: z.array(z.object({
        name: z.string(),
        // URL or local path (e.g. /img/badges/foo.png). Was `.url()`, which
        // rejected local paths and so forced partner logos to be hotlinked off
        // a third-party host — the exact failure mode the "serve from public/"
        // comments on hero_photo and logo_url exist to prevent. Every other
        // image field in this schema is a bare z.string() for that reason.
        logo_url: z.string(),
      ...
      // Copy for the /book lead page. Unset, the contractor defaults apply.
      book_page: z.object({
        eyebrow: z.string().optional(),
      ...
      estimate_form: z.object({
        heading: z.string().optional(),
        // Label for the form's address input. The field NAME stays `address`
        // (pages/api/estimate.ts requires that key), only the visible label
        // changes. Defaults to "Property address", which is right for a
        // contractor visiting a site and wrong for anyone the customer visits
        // or who ships to a business — a wholesale buyer is not giving you a
        // property.
        address_label: z.string().optional(),
        submit_label: z.string().optional(),
        address_autocomplete: z.string().optional(),
    ...
    z.object({
      ...
      promise_band: z.object({
        ...
        // Describes `image`. Same reason as blog's hero_image_alt.
        image_alt: z.string().optional(),
        // Optional mark laid over the bottom-right of `image` — a certification
        // logo that belongs ON the photo rather than in a separate badge row.
        image_badge: z.string().optional(),
        image_badge_alt: z.string().optional(),
      }).optional(),
      signature_system: z.object({
        eyebrow: z.string().optional(),
        headline: z.string(),
        intro: z.string().optional(),
        // A single diagram under the intro, as an alternative to the card grid.
        // When one image already tells the whole system story, four cards
        // restating it in prose is duplication, not reinforcement.
        image: z.string().optional(),
        image_alt: z.string().optional(),
      ...
      // Full-width video band. Sits between the promise bar and the services
      // grid on the home page. `src` may be an external URL: the client's farm
      // films are 9-28MB each, which is worth hotlinking off a CDN that serves
      // range requests rather than committing to the repo and shipping through
      // Vercel's static hosting. Always pair with a poster so a failed load
      ...
      // Third-party social-feed embed, pasted verbatim by the operator. Renders
      // after the about block. Paste-only: never synthesize one.
      social_embed: z.object({
        eyebrow: z.string().optional(),
        headline: z.string().optional(),
        snippet: z.string(),
      }).optional(),
