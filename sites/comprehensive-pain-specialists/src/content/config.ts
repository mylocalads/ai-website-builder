  ...
  schema: z.discriminatedUnion('kind', [
    z.object({
      ...
      partners: z.array(z.object({
        name: z.string(),
        // Relaxed from `.url()` to plain string so partner logos can be served
        // from this site's own public/ directory. The template's `.url()`
        // constraint forced an absolute URL, which meant either hotlinking the
        // client's old CMS (fragile — those assets can vanish) or hardcoding the
        // production hostname (breaks in dev and on preview deploys).
        logo_url: z.string(),
